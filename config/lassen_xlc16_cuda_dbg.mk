...
WORK = /p/gpfs1/vicmagri
PROJECTS = $(HOME)/projects
MFEM_CONFIG = xl-$(COMPILER_VERSION)_cuda-$(CUDA_VERSION)_umpire_rel
PREFIX = $(WORK)/projects/mfem-vm/install/$(MFEM_CONFIG)
...
HYPRE_CONFIG = xl-$(COMPILER_VERSION)_cuda-$(CUDA_VERSION)_umpire_dbg
...
ifeq ($(MFEM_USE_SUPERLU)$(MFEM_USE_STRUMPACK)$(MFEM_USE_MUMPS),NONONO)
   ifeq ($(MFEM_USE_METIS_5),NO)
     METIS_DIR = $(PROJECTS)/metis-4.0
     METIS_OPT =
     METIS_LIB = -L$(METIS_DIR) -lmetis
   else
     METIS_DIR = $(PROJECTS)/metis-5.1.0/install/xlc16_rel
   ...
   METIS_DIR = $(PROJECTS)/parmetis-4.0.3
   METIS_OPT = -I$(METIS_DIR)/include
...
SUNDIALS_DIR    = $(PROJECTS)/sundials-5.0.0/instdir
...
MESQUITE_DIR = $(PROJECTS)/mesquite-2.99
...
SUITESPARSE_DIR = $(PROJECTS)/SuiteSparse
...
# STRUMPACK >= v2.2.0)
SCOTCH_DIR = $(PROJECTS)/scotch_6.0.4
...
SCALAPACK_DIR = $(PROJECTS)/scalapack-2.0.2
...
MUMPS_DIR = $(PROJECTS)/MUMPS_5.2.0
...
STRUMPACK_DIR = $(PROJECTS)/STRUMPACK-build
...
GINKGO_DIR = $(PROJECTS)/ginkgo/install
...
PUMI_DIR = $(PROJECTS)/pumi-2.1.0
PUMI_OPT = -I$(PUMI_DIR)/include
PUMI_LIB = -L$(PUMI_DIR)/lib -lpumi -lcrv -lma -lmds -lapf -lpcu -lgmi -lparma\
   -llion -lmth -lapf_zoltan -lspr

# HIOP
HIOP_DIR = $(PROJECTS)/hiop/install
HIOP_OPT = -I$(HIOP_DIR)/include
HIOP_LIB = -L$(HIOP_DIR)/lib -lhiop $(LAPACK_LIB)

# GSLIB library
GSLIB_DIR = $(PROJECTS)/gslib/build
...
UMPIRE_CONFIG=nvcc-$(CUDA_VERSION)_$(CUDA_ARCH)_xl-$(COMPILER_VERSION)_dbg
UMPIRE_DIR = $(WORK)/projects/umpire-$(UMPIRE_VERSION)/$(UMPIRE_CONFIG)
