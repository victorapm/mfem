...
BASE_FLAGS  = -std=c++11 -I$(MPICH_DIR)/include
...
WORK = /lustre/orion/geo127/scratch/victorapm
PROJECTS = $(WORK)/projects
MFEM_CONFIG = cce-$(COMPILER_VERSION)_rocm-$(ROCM_VERSION)_rel
PREFIX = $(PROJECTS)/forks/mfem-dev/install/$(MFEM_CONFIG)
...
ifeq ($(MFEM_USE_HIP),YES)
   ...
   # Extra info to make linking work when TPLs are built via cray compiler
   CCE_DIR  = /opt/cray/pe/cce/$(COMPILER_VERSION)/cce/x86_64
   MPI_LIB += -L$(CCE_DIR)/lib $(XLINKER)-rpath,$(CCE_DIR)/lib -lu
endif
...
HYPRE_VERSION = v2.28.0-83-gaed642965
HYPRE_CONFIG  = cce-$(COMPILER_VERSION)_rocm-$(ROCM_VERSION)_mixint_rel
HYPRE_DIR = $(PROJECTS)/hypre/hypre_$(HYPRE_VERSION)_$(HYPRE_CONFIG)
HYPRE_OPT = -I$(HYPRE_DIR)/include
HYPRE_LIB = -L$(HYPRE_DIR)/lib $(XLINKER)-rpath,$(HYPRE_DIR)/lib -lHYPRE
...
ifeq (YES,$(MFEM_USE_HIP))
   ...
   HYPRE_LIB += -L$(HIP_DIR)/lib $(XLINKER)-rpath,$(HIP_DIR)/lib -lrocsparse -lrocrand -lrocsolver -lrocblas
endif
...
ifeq ($(MFEM_USE_SUPERLU)$(MFEM_USE_STRUMPACK)$(MFEM_USE_MUMPS),NONONO)
   ifeq ($(MFEM_USE_METIS_5),NO)
     ...
     METIS_DIR = $(PROJECTS)/metis-5.1.0/install/cce-$(COMPILER_VERSION)_rel
